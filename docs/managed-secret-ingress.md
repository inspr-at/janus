...
only opaque, value-free custody references.
...
bounded timestamps, and—after successful encryption—opaque binding, secret,
and generation references. It is private, size- and entry-bounded, atomically
...
creates encrypted Janus custody but does not materialize an environment file,
contact a host, register an operation with Pharos, restart a service, or claim
health. Delivery and every host effect require separate reviewed slices.
...
Only after the daemon returns that complete receipt does Go mark admission
complete with the three opaque references. The replay document contains no
...
The completed page says **Encrypted custody confirmed**, clears first-party
cache and storage, and contains no value input, reveal, copy, delivery, reload,
or health claim. The capability remains explicitly default-off.
