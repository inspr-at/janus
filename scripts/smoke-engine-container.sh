...
for binary in ("janusd", "janusd-use", "janusd-admin", "janusd-web-transactiond", "janusd-dynamic-custodyd", "janus-host-executor", "janus-warden"):
...
print("engine image filesystem ok user=65532:65532 binaries=7 runtime_packages=0 shell=none")
PY
...
if "${runtime[@]}" --entrypoint /usr/local/bin/janusd-dynamic-custodyd \
  "${image}" --help >/dev/null 2>&1; then
  echo "janusd-dynamic-custodyd unexpectedly accepted argv" >&2
  exit 1
fi
