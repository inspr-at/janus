...
func knowledgeTerms() []KnowledgeTerm {
	return []KnowledgeTerm{
		{ID: "secret", Name: "Secret", Plain: "A governed value that Janus handles without exposing it to the control plane.", Detail: "The catalog describes a secret by safe metadata. A provider owns the stored value; reviewed use paths may deliver it only to a fixed consumer.", FlowSlugs: []string{"manual-secret-entry", "generated-rotation", "approved-use", "env-file-handoff"}},
		...
		{ID: "role", Name: "Role", Plain: "A closed bundle of maximum permissions, narrowed by reviewed bindings and policy.", Detail: "The Rust ceiling and the shared role matrix agree on nine roles. A policy may remove authority but cannot add permissions beyond the compiled ceiling.", Values: roleKnowledgeValues(), FlowSlugs: []string{"approved-use", "break-glass", "role-binding-lifecycle"}},
		{ID: "permission", Name: "Permission", Plain: "One exact operation category that the role matrix may authorize.", Detail: "Permissions are the shared language between runtime actions and roles. The matrix assigns only this compiled set; it deliberately contains no reveal, audit suppression, arbitrary command, custody, or cross-scope bypass permission.", Values: permissionKnowledgeValues(), FlowSlugs: []string{"approved-use", "generated-rotation", "env-file-handoff", "host-retirement", "break-glass", "role-binding-lifecycle"}},
		...
		{ID: "lifecycle", Name: "Lifecycle", Plain: "The closed state machine that decides whether normal use is allowed.", Detail: "Only active and rotating allow normal approved use. Destruction is a value-free metadata and tombstone workflow; it does not claim provider-side deletion.", Values: []KnowledgeValue{kv("draft", "Metadata exists; normal use blocked."), kv("active", "Normal reviewed use allowed."), kv("rotating", "Rotation underway; approved use may continue."), kv("deprecated", "Migration required; normal use blocked."), kv("disabled", "Normal use blocked."), kv("pending_delete", "Awaiting destroy evidence; normal use blocked."), kv("destroyed", "Value-free tombstone/metadata only.")}, FlowSlugs: []string{"generated-rotation", "host-retirement"}},
