...
func (app *App) handleManagedDynamicValueAdmission(w http.ResponseWriter, r *http.Request) {
	...
	if !app.managedDynamicSetupEnabled() || !app.cfg.RequireAuth {
	...
	if err != nil {
		if managedDynamicValueAlreadyAdmitted(err) {
			...
			app.audit(r, "managed_environment.value.admit", "allowed", session.Subject, "duplicate resolved to existing value-free admission state")
	...
	if err := processManagedDynamicValue(r.Body, remaining, proof.Target.Source, rand.Reader); err != nil {
		app.clearManagedDynamicStepUpProofCookies(w)
		w.Header().Set("Clear-Site-Data", `"cache", "storage"`)
	...
	completed, err := app.managedDynamicSetup.CompleteValueAdmission(r.Context(), proof.Target, proof.OperationRef)
	if err != nil || completed.OperationRef != proof.OperationRef ||
		...
		app.clearManagedDynamicStepUpProofCookies(w)
		w.Header().Set("Clear-Site-Data", `"cache", "storage"`)
		app.audit(r, "managed_environment.value.admit", "denied", session.Subject, "value-free completion state unavailable")
		app.renderSafeFailure(w, r, http.StatusServiceUnavailable, "value_admission_incomplete", "Janus forgot the submitted bytes but could not confirm the value-free receipt. Start again from Pharos.", nil)
		return
	}
	w.Header().Set("Clear-Site-Data", `"cache", "storage"`)
	app.audit(r, "managed_environment.value.admit", "allowed", session.Subject, "one bounded value admitted and forgotten without custody")
	http.Redirect(w, r, "/managed-environment/setup?intent="+url.QueryEscape(proof.Target.IntentRef), http.StatusSeeOther)
}

func processManagedDynamicValue(reader io.Reader, remaining int64, source string, random io.Reader) error {
	if reader == nil || remaining < 0 || remaining > managedDynamicEncodedValueMaxBytes {
		return errors.New("managed dynamic value body invalid")
	}
	...
	if _, err := io.ReadFull(reader, rawValue); err != nil || !requestBodyAtEOF(reader) {
		return errors.New("managed dynamic value body incomplete")
	}
	...
	switch source {
	case "import":
		return validateManagedDynamicValue(value)
	case "generated":
		if len(value) != 0 {
			return errors.New("managed dynamic generated value must be internal")
		}
		generated, err := generateManagedDynamicValue(random)
		if err != nil {
			return err
		}
		defer zeroizeBytes(generated)
		return validateManagedDynamicValue(generated)
	default:
		return errors.New("managed dynamic value source invalid")
	}
}
